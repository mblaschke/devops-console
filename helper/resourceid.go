...
var (
	resourceIdRegExp = regexp.MustCompile(`(?i)^/subscriptions/(?P<subscription>[^/]+)/resourceGroups/(?P<resourceGroup>[^/]+)(/providers/(?P<resourceProvider>[^/]+)/(?P<resourceProviderNamespace>[^/]*)/(?P<resourceName>[^/]+))?/?$`)
)
